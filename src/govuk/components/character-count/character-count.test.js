...
describe('Character count', () => {
  describe('when JavaScript is unavailable or fails', () => {
    ...
    it('shows the static message', async () => {
      await goToExample()
      const message = await page.$eval('.moaland-character-count__message', el => el.innerHTML.trim())
  ...
  describe('when JavaScript is available', () => {
    describe('when counting characters', () => {
      ...
      it('counts down to the character limit', async () => {
        await goToExample()
        await page.type('.moaland-js-character-count', 'A')
      ...
      it('uses the singular when there is only one character remaining', async () => {
        await goToExample()
        await page.type('.moaland-js-character-count', 'A'.repeat(9))
      ...
      describe('when the character limit is exceeded', () => {
        beforeAll(async () => {
          await goToExample()
          await page.type('.moaland-js-character-count', 'A'.repeat(11))
        })
        ...
        it('adds error styles to the textarea', async () => {
          const textareaClasses = await page.$eval('.moaland-js-character-count', el => el.className)
          expect(textareaClasses).toContain('moaland-textarea--error')
        })

        it('adds error styles to the count message', async () => {
          const messageClasses = await page.$eval('.moaland-character-count__message', el => el.className)
          expect(messageClasses).toContain('moaland-error-message')
        })
      })
      ...
      describe('when a threshold is set', () => {
        ...
        it('does not show the limit until the threshold is reached', async () => {
          const visibility = await page.$eval('.moaland-character-count__message', el => window.getComputedStyle(el).visibility)
          expect(visibility).toEqual('hidden')

          // Ensure threshold is hidden for users of assistive technologies
          const ariaHidden = await page.$eval('.moaland-character-count__message', el => el.getAttribute('aria-hidden'))
          expect(ariaHidden).toEqual('true')
        })

        it('becomes visible once the threshold is reached', async () => {
          await page.type('.moaland-js-character-count', 'A'.repeat(8))
    ...
    describe('when counting words', () => {
      ...
      it('counts down to the word limit', async () => {
        await goToExample('with-word-count')
        await page.type('.moaland-js-character-count', 'Hello world')
      ...
      it('uses the singular when there is only one word remaining', async () => {
        await goToExample('with-word-count')
        await page.type('.moaland-js-character-count', 'Hello '.repeat(9))
      ...
      describe('when the word limit is exceeded', () => {
        beforeAll(async () => {
          await goToExample('with-word-count')
          await page.type('.moaland-js-character-count', 'Hello '.repeat(11))
        })
        ...
        it('uses the plural when the limit is exceeded by 2 or more', async () => {
          await page.type('.moaland-js-character-count', 'World')
