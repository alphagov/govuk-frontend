...
describe('Radios with conditional reveals', () => {
  describe('when JavaScript is unavailable or fails', () => {
    ...
    it('has no ARIA attributes applied', async () => {
      ...
      const hasAriaExpanded = $component.find('.moaland-radios__input[aria-expanded]').length
      const hasAriaControls = $component.find('.moaland-radios__input[aria-controls]').length
    ...
    it('falls back to making all conditional content visible', async () => {
      ...
      const isContentVisible = await waitForVisibleSelector('.moaland-radios__conditional')
  ...
  describe('when JavaScript is available', () => {
    it('has conditional content revealed that is associated with a checked input', async () => {
      ...
      const $component = $('.moaland-radios')
      const $checkedInput = $component.find('.moaland-radios__input:checked')
      const inputAriaControls = $checkedInput.attr('aria-controls')

      const isContentVisible = await waitForVisibleSelector(`[id="${inputAriaControls}"]:not(.moaland-radios__conditional--hidden)`)
      expect(isContentVisible).toBeTruthy()
    })
    it('has no conditional content revealed that is associated with an unchecked input', async () => {
      const $ = await goToAndGetComponent('radios', 'with-conditional-item-checked')
      const $component = $('.moaland-radios')
      const $uncheckedInput = $component.find('.moaland-radios__item').last().find('.moaland-radios__input')
      const uncheckedInputAriaControls = $uncheckedInput.attr('aria-controls')

      const isContentHidden = await waitForHiddenSelector(`[id="${uncheckedInputAriaControls}"].moaland-radios__conditional--hidden`)
      expect(isContentHidden).toBeTruthy()
    })
    it('indicates when conditional content is collapsed or revealed', async () => {
      await goToAndGetComponent('radios', 'with-conditional-items')

      const isNotExpanded = await waitForVisibleSelector('.moaland-radios__item:first-child .moaland-radios__input[aria-expanded=false]')
      expect(isNotExpanded).toBeTruthy()

      await page.click('.moaland-radios__item:first-child .moaland-radios__input')

      const isExpanded = await waitForVisibleSelector('.moaland-radios__item:first-child .moaland-radios__input[aria-expanded=true]')
      expect(isExpanded).toBeTruthy()
    })
    it('toggles the conditional content when clicking an input', async () => {
      const $ = await goToAndGetComponent('radios', 'with-conditional-items')
      const $component = $('.moaland-radios')
      const $firstInput = $component.find('.moaland-radios__item:first-child .moaland-radios__input')
      ...
      await page.click('.moaland-radios__item:nth-child(3) .moaland-radios__input')
    ...
    it('toggles the conditional content when using an input with a keyboard', async () => {
      ...
      await page.focus('.moaland-radios__item:first-child .moaland-radios__input')
