...
/**
 * Snapshots the node in the accessibility tree corresponding to the given element
 *
 * @param {import('puppeteer').ElementHandle} $element - Puppeteer element handle
 * @returns {Promise<import('puppeteer').SerializedAXNode>})} -
 *   The accessibility tree node corresponding to the element
 * @throws {Error} If no node corresponds to the element in the accessibility tree
 *   (for ex. if you query an AT node for a `span` with no role)
 */
function getAccessibilityTreeNode ($element) {
  const node = page.accessibility.snapshot({ root: $element })

  if (!node) {
    throw new Error('No accessibility tree node exist for this element')
  }
...
async function getAccessibleName ($element) {
  return (await getAccessibilityTreeNode($element)).name
}
