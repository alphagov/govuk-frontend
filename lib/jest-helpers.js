...
function render (componentName, params, children = false) {
  return cheerio.load(html(componentName, params, children))
}

function renderTemplate (params = {}, blocks = {}) {
  return cheerio.load(htmlTemplate(params, blocks))
}
...
/**
 * Get the raw HTML of a component
 * @param {string} componentName
 * @param {object} params parameters that are used in the component macro
 * @param {any} children any child components or text, pass the children to the macro
 * @returns {string} returns rendered string of the macro
 */
function html (componentName, params, children = false) {
  if (typeof params === 'undefined') {
    throw new Error('Parameters passed to `html` should be an object but are undefined')
  }
...
/**
 * Get the raw HTML representation of template.njk
 * @param {object} params parameters that are used in the template
 * @param {any} blocks any blocks to apply to the template
 * @returns {string} returns rendered string of the template
 */
function htmlTemplate (params, blocks) {
  ...
  return nunjucks.renderString(viewString, params)
}
...
module.exports = { render, getExamples, html, htmlTemplate, htmlWithClassName, renderSass, renderTemplate }
