...
We write functional tests for checking our JavaScript exports and our global sass variables - see [all.test.js](../../packages/govuk-frontend/src/govuk/all.test.mjs) for examples of global tests we run.
...
When you run the tests locally (for example, using `npm run test:screenshots --workspace packages/govuk-frontend-review`), Percy commands are ignored and Percy does not generate any screenshots. You will see the following message in your command line output: `[percy] Percy is not running, disabling snapshots`.
